...
Coding Agentの台頭により、AIは「聞けば答える道具」から「指示すれば実装する協働者」へ進化した。
...
AIが既存の情報を組み合わせてブロックのように組み立てる作業は得意だ。しかし「何が足りないか」「何が欠けているか」を法則性なしに直感する能力は、現時点のAIにはない。新しいアイデアとは、まだ存在しない概念を言語化する行為だ。「ないものを認識する」能力なしには生まれない。
...
Vibe Codingの普及は「誰でも作れる」という認識を広めたが、同時に「簡単にできるんでしょ」という過小評価も生んだ。動くものを作ることと、ユーザーに価値を届けるプロダクトを作ることの間には依然として大きな溝がある。この溝を埋めるのがエンジニアの専門性だ。ただし、言葉で説明するより「じゃあ作ってみて」の一言が早い。手を動かせば、その溝の深さを自分で体感できる。

=== 失敗機会の減少──「深さ」が身につかないAI時代の課題
...
具体的なガードレールには、Agent Skills（プロジェクトのルートなどに配置するMarkdownファイルで、AIが従うべきルールや参照すべき情報を定義する仕組み）の整備、使用するAPIの情報整理、プロジェクトのルール文書化などがある。たとえばプロジェクトルートにスキルファイルを配置し、AIが参照すべき開発ルールをあらかじめ用意しておく。AIはこのスキルファイルを参照することで、プロジェクト固有のルールに沿ったコードを生成できる。
...
ここで伝えたいのはテクニックそのものではなく、マインドセットの部分だ。「AIが書いたコードは正しいはず」と信じるのではなく、「AIには最新の情報がない可能性がある」と常に疑う姿勢。そして疑うだけでなく、正しい情報をAIに渡す責任は人間の側にある。
...
従来のハッカソンでは、すべてのコードを人間が手で書いていた。まずチームでアイデアを決め、役割分担を行う。フロントエンド担当、バックエンド担当、インフラ担当──それぞれがエディタを開き、コードを1行ずつ書いていく。APIのエンドポイント設計を決め、データベースのスキーマを定義し、画面レイアウトをCSSで整える。24時間のハッカソンなら、環境構築に2〜3時間、基本的なCRUD機能の実装に半日、残りの時間でUIの調整とデモの準備をする。限られた時間の中で「何を実装し、何を諦めるか」の判断が勝敗を分けた。経験年数が大きなアドバンテージとなった。
...
基本実装をAIが担う今、差がつくのは企画力と統合力だ。この統合力を支える具体的な手法が、非同期Coding Agentの活用だ。

非同期Coding Agentとは、開発者がAIにタスクを指示した後、AIがバックグラウンドで自律的にコードを書き続ける仕組みだ。人間は結果を待つ間に別の作業を進められる。
...
「手を動かす」の再定義が必要だ。かつてはキーボードでコードを打つことが「手を動かす」だった。今は、AIに指示を出し、出力を評価し、方向を修正するサイクルを回すことが「手を動かす」に変わった。能動的にAIを使い倒すこと──それが今の「手を動かす」だ。知っているだけでは価値にならない。試した人だけが次の一歩を踏み出せる。
