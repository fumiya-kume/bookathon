...
@<b>{Kuu}：そうです。「いいプロンプト@<fn>{prompt}を書く」という話ではなく、もっと上流の「何を作りたいか」「なぜ作りたいか」という願いの質が問われる。ランプの精は願いの実行は完璧にこなすけれど、願いそのものは考えてくれない。その部分だけは、AIに任せられない。
...
@<b>{Kuu}：もう一つ、AIと人間の決定的な違いがあって。AIのコンテキストウィンドウ@<fn>{context-window}って、全部の情報を同じ輝度100で均一に扱うんですよ。でも人間は違う。常に考えていることは輝度80とか90で明るくて、薄く保持しているものは輝度1とか2くらい。広い範囲に情報を持っているけれど、基本的に全部薄い。

@<b>{Lemio}：その表現すごくいいですね。
...
@<b>{Kuu}：たとえば、マーケティング担当者が「顧客データを可視化するダッシュボードがほしい」とAIに伝え、数回のやり取りでWebアプリを完成させる。営業担当者が「商談管理のツールがほしい」と言えば、簡易的なCRMが数時間で動く。ハッカソンでも、プログラミング経験のない参加者がAIと対話しながらプロトタイプを形にする場面が増えている。
...
@<b>{Kuu}：AIをそのまま使うと、古い情報でハルシネーション@<fn>{hallucination}を生み出します。だからこそ「ガードレール」という考え方が重要なんです。AIに自由に動いてもらう前に、正しい方向に走れるよう枠を作っておく。
...
@<b>{Sae}：スポンサー企業の視点から見ても、ガードレールが整ったチームの方がAIの出力品質が安定していて、短時間で成果を出しています。準備に時間をかける価値はありますね。

@<b>{Kuu}：その通りです。一度整えたガードレールはチーム全体で再利用できる。一人が整備すれば、チーム全員のAI活用の質が上がる。これは従来の「各自がコードを書く」モデルにはなかった効率化です。

=== ライブラリ依存の落とし穴

@<b>{Kuu}：JavaScriptやPythonのエコシステムでは、ライブラリの更新速度が速い。npm@<fn>{npm-pip}やpipで管理されるパッケージは日々バージョンが上がり、APIの破壊的変更も珍しくない。ここにAIが加わると、問題が構造的に悪化する。
...
@<b>{Kuu}：ガードレールを引くために、既存のCI@<fn>{ci-cd}やLint@<fn>{linter}のような仕組みは今でも使えますし、むしろ重要性が上がっている気がします。
...
@<b>{Kuu}：CI/CDはフィードバックループの高速化にも直結します。masterに直接pushする運用でCD（Continuous Deployment）を組んでおけば、開発者がpushするだけで自動デプロイ@<fn>{deploy}される。検証担当のメンバーはブラウザをリロードするだけで最新版を確認できるんです。
...
@<b>{Kuu}：もう一つ、リンターで意識してほしいのがファイルの行数制限です。1ファイルが300行、500行と膨らむと、AIがファイルを読み込むだけでトークン@<fn>{token}を大量に消費する。さらに長いファイルは複数回に分けて読む必要があり、AIの作業効率が落ちる。

@<b>{Sae}：海外のハッカソンでも、コードの構造がAIフレンドリーなチームほど開発速度が速い傾向がありました。「AIが読みやすいコード」という視点は重要ですね。

@<b>{Kuu}：その通りです。ただ、AIにとっての読みやすさは人間と少し違う。ファイルを短く保つこと、ファイル名を内容に即した具体的な名前にすること。AIはファイル名をラベルとして使って関連コードを探すので、適切な命名がそのままAIの検索精度に直結する。リンターで行数上限を設定しておけば、AIが1つのファイルに処理を詰め込み続ける事態を防げる。たとえば1ファイル200〜300行を目安にリンターで上限を設定するとよい。
...
@<b>{Kuu}：全部は無理でも、プロダクトの核になる部分だけでも書いておくと効果は大きいです。「このAPIはこの入力に対してこの出力を返すべき」というテストが1つあるだけで、AIの暴走を防げる。たとえばテストディレクトリをAIの作業ディレクトリと分離し、CIで自動実行する構成が有効だ。
...
@<b>{Kuu}：ただのアプリケーションだと本当に簡単に3秒でポッとできるので、それ以外の技術を絡めたものが求められます。
...
@<b>{Kuu}：そこがポイントです。AIがあれば「最悪ここまでは作れる」というベースラインが保証される。心理的安全性が担保された状態で冒険できる。ハッカソンのブランチ戦略としても、PR@<fn>{pull-request}（プルリクエスト）不要でmasterに直接pushし、monorepo@<fn>{monorepo}＋single branchで運用する。コードレビューもブランチ切り替えも省いて、フィードバックループを最速にすることで、挑戦と安定の両立が可能になる。
