...
対策の核心は、AIに最新の情報を渡すことである。使用するライブラリのバージョンを明示し、公式ドキュメントのURLをプロンプトに含める。APIリファレンスを起点にAIに書かせることで、古い情報に基づく生成を防げる。たとえば使用ライブラリのバージョンを明示し、公式ドキュメントのURLをプロンプトに含めることが有効だ。具体的なテクニックは@<chapref>{04-ai-tips-in-practice}で詳しく扱う。
