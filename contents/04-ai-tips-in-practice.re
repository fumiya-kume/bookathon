...
@<b>{Kuu}：プロンプトに毎回同じことを書くのは非効率です。Coding Agent@<fn>{coding-agent-ch04}には「Agent Skills@<fn>{agent-skills-ch04}」と呼ばれる仕組みがあって、プロジェクト固有のルールや慣習を事前に設定ファイルとして渡せる。これを整備しておくと、AIが毎回正しい前提で動いてくれます。
...
@<b>{Kuu}：git worktree@<fn>{git-worktree-ch04}を使います。一つのリポジトリから複数の作業ディレクトリを作って、それぞれに別のCoding Agentを割り当てる。Agent Aにはフロントエンドを、Agent Bにはバックエンドを、自分はインフラの設定をやる、という具合です。
...
@<b>{Kuu}：ハッカソンでは、あえてmasterブランチ@<fn>{branch}に直接pushする戦略を取ることがあります。通常の開発ではブランチを切ってプルリクエストを出すのが常識ですが、2〜3日の短期決戦ではそのオーバーヘッドが致命的です。

@<b>{Lemio}：コンフリクト@<fn>{conflict}の解消に時間を取られるリスクはないですか？
...
@<b>{Kuu}：CDがあることで、開発者がmasterにpushするだけで自動デプロイされ、検証者はブラウザをリロードするだけで最新版を試せます。手動では非常に手間がかかりますが、CDなら自動です。
...
@<b>{Kuu}：ElevenLabs@<fn>{elevenlabs}のボイスエージェントがまさにそうで、ユーザーが声で話した内容をトリガーにして次のアクションを実行できる。「話す」と「操作する」が一体になった、次世代のアプリケーションが作れます。
