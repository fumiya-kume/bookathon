...
@<chapref>{01-ai-native-mindset}で紹介した「ガードレール思考」を、ここでは具体的なテクニックに落とし込む。AIに正しく動いてもらうために、まず何を整えるべきかを見ていこう。
...
AIの学習データは数ヶ月前のスナップショットであり、日々更新されるライブラリのAPIとは乖離がある。公式ドキュメントのURLをプロンプトに含める手法は、AIの持つ古い知識を最新の情報で上書きする作業にほかならない。

この問題の厄介さは「動いてしまう」ことにある。非推奨APIで書かれたコードは正常に動作するが、将来のバージョンアップで突然壊れたり、セキュリティパッチが当たらなかったりする。典型的な例がある。Pythonの@<code>{requests}ライブラリではセッション管理の推奨パターンが変わった。ReactではクラスコンポーネントからHooksへの移行が完了している。しかしAIは学習データの割合に引きずられ、古い書き方で生成することがある。

もっとも即効性のある対策は、公式ドキュメントのURLをプロンプトの冒頭に記載することだ。「以下の公式ドキュメントを参照してコードを書いてください」と一文添えるだけで、AIは学習データに頼らず最新の仕様をもとにコードを生成する。あわせてバージョンを明示する。「Python 3.12、React 19を使用」と書き、@<code>{package.json}や@<code>{requirements.txt}をプロンプトに含めることで、バージョン間の整合性が取れたコードが生成される。さらに生成されたコードの@<code>{import}文を確認する習慣をつける。非推奨のモジュールからインポートしていないか、廃止されたパッケージを参照していないかをチェックする。入力品質の重要性については、本章末のアンチパターンの節で改めて扱う。
...
Agent Skills（@<chapref>{01-ai-native-mindset}で紹介した、AIが参照するプロジェクト固有のルール定義ファイル）の整備は、新メンバー向けのオンボーディング資料を書く感覚に近い。プロジェクトの規約やコーディングスタイルを、AIが読める形式で渡す。
...
「設計を言語化する」とは、頭の中にある曖昧なイメージを、AIが解釈できる精度まで具体化する行為だ。コードが書けなくても、「入力はCSVファイル、出力は月別売上のグラフ、形式はPNG」と言語化できれば、AIはそのとおりに実装する。プログラミングスキルとは無関係の能力だ。
...
非同期Coding Agentの活用は、一人の開発者が複数人分の作業を同時進行させる手法だ。Coding Agentを並列で動かせば、人間はディレクションに徹しながら、複数のタスクを同時に前進させられる。
...
ハッカソン限定の戦略として、master直pushは有効だ。@<chapref>{01-ai-native-mindset}でもマインドセットとして触れたが、ここでは具体的な運用手順に焦点を当てる。対話に出てきたCD（Continuous Deployment）と組み合わせることで、さらに効果が増す。通常の開発プロセスでは、コードレビューとブランチ管理が品質の担保に不可欠だが、ハッカソンという時間制約の下では、プロセスの厳密さよりもアウトプットの速度が優先される。
...
従来の検索は「正確なキーワードを知っている人」が有利だった。CSSの@<code>{border-radius}を知らなければ「角丸」にたどり着けなかった。しかしAIの登場で、「なんかこう、角がまるっとしてて、ちょっと浮いてる感じのカード」と言えば、@<code>{border-radius}と@<code>{box-shadow}の両方を含むコードが返ってくる。正確な技術用語を知らなくても、感覚的な表現でAIが意図を読み取る時代がすでに来ている。
...
音声AI技術は急速に進化しており、リアルタイム音声認識と音声合成を組み合わせたボイスエージェントの構築が個人開発者にも手の届く範囲になった。@<chapref>{02-idea-and-expression}の「入力と出力を定義する」思考法がここでも効く。入力を「音声」、出力を「音声＋アクション」と定義するだけで、従来とは異なるプロダクト体験が見えてくる。
...
AIの出力が期待どおりでないとき、「AIが間違えた」と結論づけるのはもっともよくあるアンチパターンだ。Lemioが言うように、これはコンパイルエラーを「叱られている」と感じる心理に似ている。エラーは敵ではなく、何が足りないかを教えてくれるシグナルだ。プロジェクトの前提条件、使用するライブラリのバージョン、期待する出力形式──これらの情報が欠けていれば、AIは推測で補うしかない。
...
対話で触れたように、AIツールは3ヶ月で別物に進化する。定期的にツールの再評価を行い、自分の「使えないリスト」を更新する習慣が重要だ。
...
AIツールについて意見を述べるなら、まず自分で触ることが前提だ。@<chapref>{02-idea-and-expression}でも述べた通り、「評論より実装」の原則はAIツールの評価にもそのまま当てはまる。少なくとも数時間は自分の手で試してから判断する習慣を持ちたい。
