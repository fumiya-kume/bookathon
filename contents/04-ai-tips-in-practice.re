...
通常の開発プロセスでは、コードレビューとブランチ管理が品質を担保する。しかしハッカソンの時間制約下では、プロセスの厳密さよりアウトプットの速度が優先される。master直pushをCD（Continuous Deployment）と組み合わせることで、Kuuが述べた「pushするだけでデプロイ、リロードするだけで検証」という高速フィードバックループが成立する。
...
CDの併用がこの戦略の鍵だ。masterへのpushをトリガーにステージング環境@<fn>{staging}へ自動デプロイする仕組みを用意しておく。これにより「開発者1人＋検証者複数人」という体制が自然に成立し、コードを書かないメンバーも即座にフィードバックを返せる。
...
@<b>{Kuu}：@<em>{音声入力の利点は、言語化のステップを省略できることです。頭の中のイメージをテキストに変換する手間がなく、Vibeでそのまま出てくる。}

@<b>{Sae}：@<em>{膨大な情報をAIに伝えたいとき、音声だと入れやすいんですよね。タイピングだと億劫になる量でも、話すなら自然に伝えられる。}

@<b>{Lemio}：@<em>{しかも音声なら擬音語やオノマトペを使って、ふわっとした形から絞り込んでいける。昔見たYouTubeの動画で、タイトルが全然わからなくても、「こういう感じのやつ」と探していったら出てきたりするんですよ。}
...
アンチパターン	ありがちな行動	改善の方向
--------------
情報不足で見切る	期待と違う出力で諦める	何の情報が足りないか分析する
自分の方が賢いと思う	一行ずつ細かく指定する	ゴールを示し道筋はAIに任せる
一度ダメだと切り捨てる	3ヶ月前の評価で止まる	定期的に再評価する
無料枠だけで判断する	制限付きの体験で実力を測る	1ヶ月だけ有料で試す
触らずに評論する	使わずに意見だけ述べる	まず数時間は自分で触る
//}

@<b>{Sae}：@<em>{ハッカソンの参加者を見ていると、AIに十分な情報を与えずに「使えない」と判断してしまう人が多いんです。十分な情報を渡していないからチグハグな出力になるのであって、AIが間違えたわけではない。}

@<b>{Lemio}：@<em>{コンパイルエラーを「叱られてる」と思う感覚に似ていますね。エラーは敵ではなく、何が足りないかを教えてくれている。AIの出力も同じで、期待と違ったら「何を伝え忘れたか」を考える方が建設的です。}

@<b>{Sae}：@<em>{一度ダメだったからといって諦めるのはもったいないんです。情報を足して再挑戦するだけで、結果がガラッと変わることがある。}

//blankline

プロジェクトの前提条件、使用するライブラリのバージョン、期待する出力形式──これらの情報が欠けていれば、AIは推測で補うしかない。出力がチグハグになるのは当然だ。

Saeが言うように、一度ダメだったからと諦めるのがもったいない。足りなかった情報を補って再度試すだけで、結果が劇的に変わることは珍しくない。うまくいかなかった経験こそが、次のプロンプト設計を磨く材料になる。

//blankline

@<b>{Lemio}：@<em>{AIが自分より頭がいいことを受け入れた方がいいんです。お膳立てしすぎて、一行ずつ指示するのは逆効果。自分が知らない領域で細かく指示しても、それはベストプラクティスにはならない。}

@<b>{Kuu}：@<em>{わかります。まず「どうやるのがベストか調べて」と聞いてから、一緒に進める方がずっとうまくいきますよね。}

@<b>{Lemio}：@<em>{そうそう。AIを赤ちゃんに教えるみたいに扱うのではなく、頼れるパートナーとして任せる。ゴールだけ示して、道筋はAIに考えさせる方がうまくいきます。}

//blankline

AIに対して過度に細かい指示を出し続けるのもアンチパターンだ。一行ずつコードを指定するような使い方は、自分で書くのと変わらない。Lemioが指摘するように、特に自分の専門外の領域では、細かく指示するほどベストプラクティスから外れていく。AIの強みは、ゴールを示せばそこまでの道筋を自律的に考えられることにある。
...
@<b>{Sae}：@<em>{AI系のツールは進化が速すぎて、3ヶ月前の評価がまったく通用しないですよね。海外のハッカソンでも、半年前にはなかったツールがいきなりスタンダードになっていることがあります。}

@<b>{Kuu}：@<em>{「あれは使えなかった」という記憶を定期的にリセットした方がいい。定期的に触り直す「健康診断」みたいな習慣が大事です。}
...
@<b>{Sae}：@<em>{無料枠で判断する人が多いですが、有料プランでないと見えない世界がある。ハッカソンに出るなら、1ヶ月だけでも有料プランを試してみてほしいです。課金しても、そこで得た経験や学びの方がはるかに価値がありますから。}

@<b>{Kuu}：@<em>{ツールへの投資って、自分のスキルへの投資でもありますよね。}

//blankline

無料枠と有料プランでは、応答速度、コンテキスト長、利用可能なモデルが大きく異なる。無料枠の体験で「AIは大したことない」と結論づけるのは、試乗車の速度制限でスポーツカーの性能を判断するようなものだ。Saeが言うように、課金額よりもそこで得られる経験の方が価値は大きい。1ヶ月だけ有料プランを試せば、AIの本来の能力を正しく評価できる。

//blankline

@<b>{Kuu}：@<em>{「AIはまだ使えない」「あのツールは微妙」と言う人に限って、自分では触っていないんですよね。手を動かさないと何もわからない。}

@<b>{Sae}：@<em>{ハッカソンの参加者を見ていても、実際に手を動かした人とそうでない人では、フィードバックの解像度がまるで違います。触った人の意見には説得力がありますね。}

//blankline

AIツールについて意見を述べるなら、まず自分で触ることが前提だ。「評論より実装」の原則は、AIツールの評価にも当てはまる。少なくとも数時間は自分の手で試してから判断する。

本章で紹介したテクニックは、ハッカソンだけでなく日常の開発でも活きる。公式ドキュメントのURLをプロンプトに添える、入力と出力を三行で書き出す、AIに「ベストプラクティスを3つ提案して」と聞いてから実装に入る──どれも今日から始められることだ。すべてを一度に取り入れる必要はない。まず一つ試して、手応えを感じたら次のテクニックへ進む。うまくいかなくても、情報を足して再挑戦する。ツールの評価を定期的にリセットする。その繰り返しが、AIを「たまに使うツール」から「頼れるパートナー」に変えていく。ハッカソンは、そのサイクルを短期間で何周も回せる最高の実験場だ。
