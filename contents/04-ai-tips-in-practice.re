...
この問題の厄介さは「動いてしまう」ことにある。非推奨APIで書かれたコードは正常に動作するが、将来のバージョンアップで突然壊れたり、セキュリティパッチが当たらなかったりする。たとえばPythonの@<code>{requests}ライブラリではセッション管理の推奨パターンが変わり、ReactではクラスコンポーネントからHooksへの移行が完了した。しかしAIは学習データの割合に引きずられ、古い書き方で生成することがある。

//note[AIの「古い知識」を防ぐ最速の方法]{
公式ドキュメントのURLをプロンプトの冒頭に記載し、「以下の公式ドキュメントを参照してコードを書いてください」と一文添える。あわせて@<code>{package.json}や@<code>{requirements.txt}をプロンプトに含め、バージョンを明示する。
//}

もっとも即効性のある対策は、公式ドキュメントのURLをプロンプトの冒頭に記載することだ。「以下の公式ドキュメントを参照してコードを書いてください」と一文添えるだけで、AIは学習データに頼らず最新の仕様をもとにコードを生成する。

あわせてバージョンを明示する。「Python 3.12、React 19を使用」と書き、@<code>{package.json}や@<code>{requirements.txt}をプロンプトに含めることで、バージョン間の整合性が取れたコードが生成される。
...
この仕組みの利点は、再現性にある。プロンプトに毎回同じ指示を書く代わりに、設定ファイルに一度書いておけばよい。以降のすべてのやり取りで、AIがその前提を踏まえて動く。チームメンバー全員が同じ設定ファイルを共有すれば、AIの出力品質がチーム全体で均一化される。
...
Coding Agentを並列で動かせば、人間はディレクションに徹しながら、複数のタスクを同時に前進させられる。

git worktreeは、一つのリポジトリから複数の作業ツリーを作成するGitの機能だ。通常のブランチ切り替えと異なり、物理的に別のディレクトリとして存在する。そのため、それぞれのディレクトリで独立してCoding Agentを起動できる。メインのworktreeでは自分がUI設計を進め、別のworktreeではAgent AがAPI実装を進め、さらに別のworktreeではAgent Bがテストコードを書く──こうした並列作業が可能になる。
...
ハッカソン限定の戦略として、master直pushは有効だ。@<chapref>{01-ai-native-mindset}でもマインドセットとして触れたが、ここでは具体的な運用手順に焦点を当てる。対話に出てきたCD（Continuous Deployment）と組み合わせることで、さらに効果が増す。
